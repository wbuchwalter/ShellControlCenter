                        ...
                        function(vt100, current, begin, count) {
                            ...
                            return function() {
                                ...
                                for (i = 0; i < this.console.length; ++i) {
                                    for (var line = this.console[i].firstChild; line; line = line.nextSibling) {
...
VT100.prototype.addKeyBinding = function(elem, ch, key, CH, KEY) {
    ...
    var clicked =
        ...
        function(vt100, elem, ch, key, CH, KEY) {
            return function(e) {
                if (vt100.lastSelectedKey) {
                    if (elem == vt100.lastSelectedKey) {
...
VT100.prototype.initializeKeyboard = function() {
    ...
    this.keyboard.innerHTML =
        this.layout.contentDocument.getElementById("siabSupport").innerHTML;
    var box = this.keyboard.firstChild;
    ...
    this.addListener(box, 'mouseout',
        function(vt100) {
            return function(e) {
                return vt100.resetLastSelectedKey(e);
            };
        }(this));
...
VT100.prototype.initializeElements = function(container) {
    ...
    if (container) {
        ...
        document.getElementById("siabSupport").appendChild(this.container);
    }
    ...
    this.lineheight = this.getChildById(this.container,
        'lineheight');
    this.console = [this.getChildById(this.container, 'console'),
        this.getChildById(this.container, 'alt_console')
    ];
    ...
    this.isIE = false; //ieProbe.offsetTop > 1;
    ieProbe = undefined;
    ...
    var marginTop = parseInt(this.getCurrentComputedStyle(
        document.getElementById("siabSupport"), 'marginTop'));
    var marginLeft = parseInt(this.getCurrentComputedStyle(
        document.getElementById("siabSupport"), 'marginLeft'));
    var marginRight = parseInt(this.getCurrentComputedStyle(
        document.getElementById("siabSupport"), 'marginRight'));
    ...
    this.isEmbedded = marginTop != y ||
        ...
        (window.innerWidth ||
            ...
            document.getElementById("siabSupport").clientWidth) -
    ...
    if (!this.isEmbedded) {
        ...
        document.getElementById("siabSupport").style.margin = '0px';
        try {
            document.getElementById("siabSupport").style.overflow = 'hidden';
        } catch (e) {}
        try {
            document.getElementById("siabSupport").oncontextmenu = function() {
                return false;
            };
        } catch (e) {}
    }
...
VT100.prototype.resizer = function() {
    ...
    // still get confused if somegetElementById("siabSupport") enters a character that is wider/narrower
    ...
    var height = (this.isEmbedded ? this.container.clientHeight : (window.innerHeight ||
        document.documentElement.clientHeight ||
        document.getElementById("siabSupport").clientHeight)) - 1;
...
VT100.prototype.mouseEvent = function(event, type) {
    ...
    var button = type != 0 /* MOUSE_DOWN */ ? 3 :
        typeof event.pageX != 'undefined' ? event.button : [undefined, 0, 2, 0, 1, 0, 1, 0][event.button];
    if (button != undefined) {
        if (event.shiftKey) {
            button |= 0x04;
        }
...
VT100.prototype.scrollFore = function() {
    ...
    this.scrollable.scrollTop = i > this.numScrollbackLines *
        this.cursorHeight + 1 ? this.numScrollbackLines *
        this.cursorHeight + 1 : i;
};
...
VT100.prototype.keysPressed = function(ch) {
    for (var i = 0; i < ch.length; i++) {
        ...
        this.vt100(c >= 7 && c <= 15 ||
            c == 24 || c == 26 || c == 27 || c >= 32 ? String.fromCharCode(c) : '<' + c + '>');
    }
};
...
VT100.prototype.handleKey = function(event) {
    ...
    if (ch != undefined) {
        ...
        if ((event.altKey || event.metaKey) && !event.shiftKey && !event.ctrlKey) {
            ...
            switch (key) {
                case 33:
                    /* Page Up      */ this.scrollBack();
                    return;
                case 34:
                    /* Page Down    */ this.scrollFore();
                    return;
                default:
                    break;
            }
        ...
        if (ch == undefined) {
            switch (key) {
                ...
                case 36:
                    ...
                    break;
                case 37:
                    /* Left         */ ch = this.cursorKeyMode ?
                        '\u001BOD' : '\u001B[D';
                    break;
                case 38:
                    /* Up           */ ch = this.cursorKeyMode ?
                        '\u001BOA' : '\u001B[A';
                    break;
                case 39:
                    /* Right        */ ch = this.cursorKeyMode ?
                        '\u001BOC' : '\u001B[C';
                    break;
                case 40:
                    /* Down         */ ch = this.cursorKeyMode ?
                        '\u001BOB' : '\u001B[B';
                    break;
                case 45:
                    /* Insert       */ ch = '\u001B[2~';
                ...
                case 93:
                    /* Select       */ return;
                case 96:
                    /* 0            */ ch = this.applyModifiers(48, event);
                    break;
                case 97:
                    /* 1            */ ch = this.applyModifiers(49, event);
                    break;
                case 98:
                    /* 2            */ ch = this.applyModifiers(50, event);
                    break;
                case 99:
                    /* 3            */ ch = this.applyModifiers(51, event);
                    break;
                case 100:
                    /* 4            */ ch = this.applyModifiers(52, event);
                    break;
                case 101:
                    /* 5            */ ch = this.applyModifiers(53, event);
                    break;
                case 102:
                    /* 6            */ ch = this.applyModifiers(54, event);
                    break;
                case 103:
                    /* 7            */ ch = this.applyModifiers(55, event);
                    break;
                case 104:
                    /* 8            */ ch = this.applyModifiers(56, event);
                    break;
                case 105:
                    /* 9            */ ch = this.applyModifiers(58, event);
                    break;
                case 106:
    ...
    if (event.shiftKey || event.ctrlKey || event.altKey || event.metaKey) {
        ...
        if ((start = ch.substr(0, 2)) == '\u001B[') {
            for (part1 = start; part1.length < ch.length &&
        ...
        this.cursorInterval = setInterval(
            function(vt100) {
                return function() {
                    ...
                    // key, or whether somegetElementById("siabSupport") pasted text into the textfield.
...
VT100.prototype.getTransformStyle = function(transform, scale) {
    return scale && scale != 1.0 ? transform == 'filter' ? 'progid:DXImageTransform.Microsoft.Matrix(' +
        'M11=' + (1.0 / scale) + ',M12=0,M21=0,M22=1,' +
        "sizingMethod='auto expand')" : 'translateX(-50%) ' +
        'scaleX(' + (1.0 / scale) + ') ' +
        'translateX(50%)' : '';
};
...
VT100.prototype.openPrinterWindow = function() {
    ...
    try {
        if (!this.printWin || this.printWin.closed) {
            ...
            this.printWin.document.getElementById("siabSupport").innerHTML =
    ...
    rc &= this.printWin && !this.printWin.closed &&
        (this.printWin.innerWidth ||
            ...
            this.printWin.document.getElementById("siabSupport").clientWidth) > 1;
            ...
            return function() {
                if (!win || win.closed ||
                    (win.innerWidth ||
                        ...
                        win.document.getElementById("siabSupport").clientWidth) <= 1) {
    ...
    for (var i = 0; i <= this.npar; i++) {
        switch (this.par[i]) {
            ...
            case 1:
                ...
                break;
            case 2:
                this.attr = (this.attr & ~0x0800 /* ATTR_BRIGHT */ ) | 0x0400 /* ATTR_DIM */ ;
                break;
            case 4:
                this.attr |= 0x0200 /* ATTR_UNDERLINE */ ;
                break;
            case 5:
                this.attr |= 0x1000 /* ATTR_BLINK */ ;
                break;
            case 7:
                this.attr |= 0x0100 /* ATTR_REVERSE */ ;
                break;
            case 10:
                this.translate = this.GMap[this.useGMap];
                this.dispCtrl = false;
            ...
            case 21:
            case 22:
                this.attr &= ~(0x0800 /* ATTR_BRIGHT */ | 0x0400 /* ATTR_DIM */ );
                break;
            case 24:
                this.attr &= ~0x0200 /* ATTR_UNDERLINE */ ;
                break;
            ...
            case 27:
                ...
                break;
            case 38:
                this.attr = (this.attr & ~(0x0400 /* ATTR_DIM */ | 0x0800 /* ATTR_BRIGHT */ | 0x0F)) |
                    0x0200 /* ATTR_UNDERLINE */ ;
                break;
            case 39:
                this.attr &= ~(0x0400 /* ATTR_DIM */ | 0x0800 /* ATTR_BRIGHT */ | 0x0200 /* ATTR_UNDERLINE */ | 0x0F);
                break;
            case 49:
                this.attr |= 0xF0;
                break;
...
VT100.prototype.csiP = function(number) {
    ...
    this.scrollRegion(this.cursorX + number, this.cursorY,
        this.terminalWidth - this.cursorX - number, 1, -number, 0, this.color, this.style);
    needWrap = false;
};
            ...
            switch (this.isEsc) {
                case 1 /* ESesc */ :
                    ...
                    switch (ch) {
                        /*%*/
                        case 0x25:
                            this.isEsc = 13 /* ESpercent */ ;
                            break;
                            /*(*/
                        case 0x28:
                            this.isEsc = 8 /* ESsetG0 */ ;
                            break;
                            /*-*/
                        case 0x2D:
                            /*)*/
                        case 0x29:
                            this.isEsc = 9 /* ESsetG1 */ ;
                            break;
                            /*.*/
                        case 0x2E:
                            /***/
                        case 0x2A:
                            this.isEsc = 10 /* ESsetG2 */ ;
                            break;
                            /*/*/
                        case 0x2F:
                            /*+*/
                        case 0x2B:
                            this.isEsc = 11 /* ESsetG3 */ ;
                            break;
                            /*#*/
                        case 0x23:
                            this.isEsc = 7 /* EShash */ ;
                ...
                case 15 /* ESnonstd */ :
                    switch (ch) {
                        ...
                        case 0x50:
                            ...
                            this.isEsc = 16 /* ESpalette */ ;
                            break;
                            /*R*/
                        case 0x52: // Palette support is not implemented
                            this.isEsc = 0 /* ESnormal */ ;
                            break;
                        default:
                            this.isEsc = 0 /* ESnormal */ ;
                ...
                case 16 /* ESpalette */ :
                    if ((ch >= 0x30 /*0*/ && ch <= 0x39 /*9*/ ) ||
                        (ch >= 0x41 /*A*/ && ch <= 0x46 /*F*/ ) ||
                        (ch >= 0x61 /*a*/ && ch <= 0x66 /*f*/ )) {
                        this.par[this.npar++] = ch > 0x39 /*9*/ ? (ch & 0xDF) - 55 : (ch & 0xF);
                        if (this.npar == 7) {
                    ...
                    if (ch == 0x3B) {
                        ...
                        switch (ch) {
                            ...
                            case 0x63:
                                ...
                                /*m*/
                            case 0x6D:
                                /* (re)set key modifier resource values */ break;
                                /*n*/
                            case 0x6E:
                                /* disable key modifier resource values */ break;
                                /*p*/
                            case 0x70:
                                /* set pointer mode resource value */ break;
                            default:
                                break;
                        }
                        this.isEsc = 0 /* ESnormal */ ;
                ...
                case 4 /* ESgotpars */ :
                    ...
                    switch (ch) {
                        /*!*/
                        case 0x21:
                            this.isEsc = 12 /* ESbang */ ;
                            break;
                            /*>*/
                        case 0x3E:
                            if (!this.npar) this.isEsc = 5 /* ESdeviceattr */ ;
                            break;
                            /*G*/
                        case 0x47:
                            /*`*/
                        ...
                        case 0x43:
                            /*a*/
                        case 0x61:
                            this.gotoXY(this.cursorX + (this.par[0] ? this.par[0] : 1),
                                this.cursorY);
                            break;
                            /*D*/
                        case 0x44:
                        ...
                        case 0x6E:
                            ...
                            break;
                            /*q*/
                        case 0x71: // LED control not implemented
                            break;
                            /*r*/
                        case 0x72:
...
ShellInABox.prototype.sendRequest = function(request) {
    ...
    request.open('POST','/api/shells/5/?', true);
